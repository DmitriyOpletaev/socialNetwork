...
export const youtubeAPIKey = 'AIzaSyBey2mu4mayQvY4yJYoAEc2cFT_PzIKzd4';
...
export const YoutubeAPI = {
    getVideosByTerm(searchTerm: string, prevPageToken: string | null, nextPageToken: string | null) {
        const pageToken = !prevPageToken && !nextPageToken ? '' : `&pageToken=${prevPageToken || nextPageToken || ''}`
    ...
    youtubes(token:string) {


        return fetch(`https://www.googleapis.com/youtube/v3/channels?key=${token}&part=snippet&mine=true`).then(
            res => {
                return res.json()
